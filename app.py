...
def header_section(option):
    ...
    logging.info(f"Header set for {option} Card registration.")
    ...
    if image_file is not None:
        ...
        if face_image is not None:
            ...
            if is_face_verified:
                ...
                # Safe date handling
                if 'DOB' in text_info:
                    text_info['DOB'] = format_dob(text_info['DOB'])
                else:
                    text_info['DOB'] = ""
                    logging.warning("DOB field missing in extracted text")
                
                records = fetch_records(text_info)
                if records.shape[0] > 0:
                    st.write(f"Found {records.shape[0]} existing records")
